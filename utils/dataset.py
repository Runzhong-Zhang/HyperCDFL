...
class Dataset(object):
    ...
    def __init__(self, base_path, video_list, label2index, shuffle = False):
        ...
        for video in video_list:
            ...
            self.hyperfeatures[video] = np.load(base_path + '/hyperfeatures/' + video + '.npy')
            # transcript
    ...
    def __next__(self):
        if self.idx == len(self):
            ...
            self.idx += 1
            return self.features[video], self.transcript[video], torch.from_numpy(self.hyperfeatures[video]).float()
