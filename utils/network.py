...
class Net(nn.Module):
    ...
    def __init__(self, input_dim, hidden_size, n_classes):
        ...
        self.z_general = Parameter(torch.fmod(torch.randn(512).cuda(),2))
        self.hypernet = HyperNetwork()


    def forward(self, x, z_specific):
        weight_general = self.hypernet(self.z_general)
        dummy, output = self.gru(x)
        output = F.linear(output, weight=weight_general)
...
#            offset += output.shape[1]
    ...
    def forward(self, sequence, z, batch_size = 512):
        data_wrapper = DataWrapper(sequence, window_size = 21)
        return self._forward(data_wrapper, z)
...
class Trainer(Forwarder):
    ...
    def train(self, sequence, transcript, z_specific, batch_size = 512, learning_rate = 0.1, window = 20, step = 5):
        #print('--------------------new video-----------------')
        data_wrapper = DataWrapper(sequence, window_size = 21)
        # forwarding and Viterbi decoding
        log_probs_origin = self._forward(data_wrapper, z_specific)
